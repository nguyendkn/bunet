...
;(function (
  this: any,
  factory: (exporter: <K extends keyof typeof Reflect>(key: K, value: (typeof Reflect)[K]) => void) => void
) {
  const root =
  ...
  const _Map: typeof Map =
    !usePolyfill && typeof Map === 'function' && typeof Map.prototype.entries === 'function' ? Map : CreateMapPolyfill()
  const _Set: typeof Set =
    !usePolyfill && typeof Set === 'function' && typeof Set.prototype.entries === 'function' ? Set : CreateSetPolyfill()
  ...
  function ToPrimitive(input: any, PreferredType?: Tag): undefined | null | boolean | string | symbol | number {
    ...
    const hint: 'string' | 'number' | 'default' =
      PreferredType === Tag.String ? 'string' : PreferredType === Tag.Number ? 'number' : 'default'
  ...
  function CreateMapPolyfill(): MapConstructor {
    ...
    return class Map<K, V> {
      ...
      private _cacheKey: K | typeof cacheSentinel = cacheSentinel
